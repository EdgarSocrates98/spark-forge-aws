...
| 6 | Primeira passada da suite: 2 falhas. `test_agents_parity::TestNoPlatformKnowledge` pegou `runtime_scope:`/`retrieved:` no exemplo YAML do DESIGN (`.claude/` e diretorio de plataforma); `test_facts_scan::test_nenhum_modulo_varre_com_glob_cru` pegou `glob` cru em `packs/load.py` | O DESIGN aponta o arquivo de regras em vez de copia-lo; `_rule_files` passou a `iter_source_files` (denylist e teto) + `resolve_within`. Trocar por `iterdir` para escapar do gate seria a evasao que ele existe para pegar |

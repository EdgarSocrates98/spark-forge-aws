...
| `SF-LF-011` | o job falhou com `ERR-LF-001` e a permissão que a operação exige **não está no grant** (sob FGAC, na escrita: **não está na decisão de IAM** do runtime role) — só sai com o log do passo 4 e o código fundidos a esta coleta |
...
| Assinatura | Regra |
|---|---|
| `Insufficient Lake Formation permission(s) on` | `SF-ERR-006`, e `SF-LF-011` quando os três lados estão na fusão: a operação (código), o modelo de acesso (Terraform ou conf) e o lado medido (grant sob FTA ou na leitura; registro da localização, runtime e decisão de IAM na escrita sob FGAC) |
...
job). Sem o código, a operação não é lida e o fact sai
`lakeformation.missing_grant.unresolved` com o motivo; sem o modelo de acesso
(`facts_tf.json`), idem; sem o lado medido — o grant sob FTA ou na leitura, e sob
FGAC na escrita o registro da localização, a versão do runtime e a decisão de IAM
do role —, idem. O lado decide o conserto:

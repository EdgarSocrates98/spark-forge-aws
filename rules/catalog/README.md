...
|---|---|---|
| `id` | sim | Único. **Vinte e uma** áreas neste README — o catálogo inteiro tem mais, porque a expansão agêntica acrescentou uma área `structural` por domínio de coordenação; conte com `area_of` em vez de somar esta lista: `PY` (PySpark), `GLUE` (infra/IaC), `UI` (Spark UI), `ICE` (Iceberg), `PQ` (Parquet/S3), `ATH` (Athena), `ENV` (ambiente/versão), `PLAN` (plano físico), `CG` (grafo de chamadas), `EMR` (cluster EMR on EC2), `EMRS` (application EMR Serverless), `EMRK` (execução de job EMR on EKS), `DQ` (validação de dados), `BENCH` (comparação entre execuções), `FVAL` (validação funcional de uma mudança), `GRAPH` (processamento de grafo com GraphFrames), `MIG` (compatibilidade de migração entre versões de runtime), `SPARK4` (compatibilidade do código com o Apache Spark 4), `KMS` (encriptação e a rota até o KMS), `NET` (conectividade de VPC) e `XACC` (leitura de catálogo entre contas). **As três últimas nasceram juntas**, para os três eixos que `sparkforge/migration/assessment.py` nomeava e ninguém preenchia — uma área única deixaria os três empatados no mesmo balde, e a separação que o contrato pede voltaria a ser decorativa. **`EMR` é prefixo de `EMRS` e de `EMRK`**: quem comparar área por `startswith` sobre o `id` conta toda regra `SF-EMRS-*` e `SF-EMRK-*` como `SF-EMR`, e a fronteira entre as três passa a ser medida ao contrário — compare pelo `area:` declarado no cabeçalho do arquivo |
...
| `emr-eks.yaml` | `SF-EMRK-*` | Fase EMR on EKS (dump de `describe-job-run` mais `describe-virtual-cluster`) |

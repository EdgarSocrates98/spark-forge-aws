...
4. `sparkforge judge --facts .sparkforge/facts_<perfil>.json --glue <versão> --show-skipped` **em cada perfil separadamente**. Não julgue um `facts.json` que misture execuções de perfis diferentes — a mesma regra significa coisas opostas em cada um.

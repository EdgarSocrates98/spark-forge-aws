...
def test_route_selects_minimal_specialist():
    ...
    route = controller.route(
        goal="inspect spark plan",
        available_agents=["sf-inventory", "sf-judge", "sf-verifier"],
        completed=set(),
    )
...
def test_route_advances_to_verification_after_analysis():
    controller = AutonomyController()
    route = controller.route(
        goal="validate finding",
        available_agents=["sf-judge", "sf-verifier"],
        completed={"inventory", "collect", "analyze", "judge"},
    )
...
def test_stop_on_stagnation_and_authorization_policy():
    ...
    sem_aprovacao = controller.authorize_tool(
        agent="a", tool="write", allowed_tools=["write"], mutating=True
    )
    com_aprovacao = controller.authorize_tool(
        agent="a", tool="write", allowed_tools=["write"], mutating=True, approval=True
    )
    assert sem_aprovacao[0] is False
    assert com_aprovacao[0] is True

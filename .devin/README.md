...
| Arquivo | O que faz |
|---|---|
| `mcp_config.json` | expõe as **111 tools** por stdio (recontado em 2026-09-20). Sem variável de ambiente — o `.mcp.json` da raiz é do plugin do Claude Code e usa `${CLAUDE_PLUGIN_ROOT}`, que nenhuma página do Devin documenta expandir |
...
As **51 skills** em `.agents/skills/` são gatilhos para isso: cada uma abre
...
**`.devin/agents/`.** Mesma razão — os 12 coordenadores e 5 executores estão em
`.agents/agents/`.

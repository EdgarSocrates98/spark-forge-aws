...
Baixa a configuracao de um workgroup via `athena.get_work_group` (engine version efetiva/selecionada, state, bytes_scanned_cutoff, output_location) e registra no manifesto, ja no shape que `sparkforge_analyze_athena_workgroup` le. Mesma politica offline-first de `sparkforge_collect_event_log`.

...
# Agent `athena-query-optimizer`

Custo ou latencia na consulta Athena e nao no job - bytes escaneados, pruning de particao, projecao de coluna, versao do engine, workgroup, layout de armazenamento.

| Campo | Valor |
|---|---|
| Papel | coordenador |
| Arquivo de origem | `agents/athena-query-optimizer.md` |
| Ferramentas do host | Read, Grep, Glob, Bash, Edit, Write |
| Áreas de regra | SF-ATH, SF-PQ |

## Skills que ele usa

[`optimize-parquet-layout`](../skills/optimize-parquet-layout.md), [`optimize-iceberg-table`](../skills/optimize-iceberg-table.md), [`benchmark-pyspark-job`](../skills/benchmark-pyspark-job.md)

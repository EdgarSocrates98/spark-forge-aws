...
Investigacao ponta a ponta de job ou biblioteca Glue PySpark com fluxos full e incremental - latest-per-key em Iceberg bilionario, batching, OOM apos horas, carga variavel. Coordena skills em vez de tuning localizado.

| Campo | Valor |
|---|---|
| Papel | coordenador |
| Arquivo de origem | `agents/glue-incremental-performance-architect.md` |
| Ferramentas do host | Read, Grep, Glob, Bash, Edit, Write |
| Áreas de regra | SF-PY, SF-ICE, SF-UI, SF-ENV |

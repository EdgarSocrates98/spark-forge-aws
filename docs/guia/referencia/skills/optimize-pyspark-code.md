...
| Campo | Valor |
|---|---|
| Arquivo de origem | `skills/optimize-pyspark-code/SKILL.md` |
...
A escada de reescrita desta skill — nativa → higher-order → `pandas_udf` medido → `udf`
Python — é uma escada de mudança de semântica: nulo, precisão decimal, fuso, ordenação de
string e comportamento de borda diferem entre a UDF e a função nativa que a substitui. E
remover hint de `broadcast` muda o particionamento e a ordem da saída, e com ela o que
`dropDuplicates`, `first`, `collect_list` e `monotonically_increasing_id` devolvem — **sem
mudar contagem nenhuma**. Reescrita que preserva a contagem e troca a linha passa nos quatro
eixos.

...
| Fixtures golden | **193** em 23 domínios | `fixtures/` |
...
| Três eixos do contrato de migração — `iam_kms`, `rede` e `cross_account` — nascem `BLOCKED` e **nunca mudam de valor** | Fase H2, medida ao nomear as etapas da §32 do prompt de migração | **Fase, não dívida — é extrator que falta, não código quebrado.** Os três estão no contrato com a evidência que os preencheria escrita ao lado, e o gate de migração os reporta em toda saída. Fechar cada um exige um extrator (IAM/KMS, rede, compartilhamento entre contas) e uma área de regra; enquanto isso, `BLOCKED` com motivo é a resposta honesta, e é diferente de `PASS`. Sem posição na *Ordem* |

### Limites declarados (26)
...
| `mig.table_format` não carrega identidade de tabela, então o eixo `consumidor` é do **job**, não da tabela | Fase H3, medido ao ligar o inventário à matriz | **Limite declarado.** O extrator observa `format-version` numa linha de fonte Python; o subject é a linha, não a tabela. O gate responde "esta migração vai escrever v3, e quem lê isso?" — não "esta tabela específica quebra para este consumidor". Fechar exige correlacionar código com nome de tabela, que é fusão, não extrator novo. Quando há dump de metadados, `iceberg.table_property` traz a identidade e `SF-ENV-002` acusa por tabela |
| `collect()` não lê dump de metadados Iceberg, então a análise a partir do diretório do job nunca tem `iceberg.table_property` | Fase H2, decisão registrada no próprio módulo | **Limite declarado.** `collect()` compõe o que o diretório do job **é** — código, `.jar`, `requirements*.txt`, `.tf` e o inventário na convenção. Um dump de metadados é coleta contra AWS viva, com outro ciclo de vida. Fechar exige decidir uma convenção de caminho para o dump, e a decisão é de escopo de coleta, não de composição |

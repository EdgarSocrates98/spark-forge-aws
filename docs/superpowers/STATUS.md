...
| Módulos da camada agêntica | **14** (+ `__init__.py`) — `models`, `runtime`, `evidence`, `blackboard`, `debate`, `arbitration`, `experiment`, `decision`, `memory`, `budget`, `security`, `autonomy`, `graph`. **Deixou de ser biblioteca inerte em 2026-09-08**: o subpacote `executor/` (10 módulos em 2026-09-11, fora desta contagem, que mede só `sparkforge/agentic/*.py`) escreve `Claim`/`Evidence`/`Contradiction`/`Unknown`/`Decision`, e num case rodado `blackboard summary` deixa de devolver zero. O executor de **debate** existe desde 2026-09-11 (`executor/debate_run.py` e `executor/debate_evidence.py`, L0, geração só no host). `AGENTS.md` publicou "12 módulos" sobre uma tabela de 13 até a auditoria de 2026-09-03 | `sparkforge/agentic/*.py` | O **14º** é `referee` (2026-09-10): ele arbitra o protocolo de debate e NÃO o executa. `_arbitrar` é a única implementação do julgamento, e as duas formas de entrada — um `Debate` em memória e o blackboard que `arbitrate` escreve em JSONL — passam por ela. Duas cópias do mesmo julgamento é o defeito que `ORDEM_DE_SEVERIDADE` teve por um dia, e um teste conta as definições de `_arbitrar` para impedi-lo.
...
Collector do operador, pelo receiver `otlpjsonfile`.
...
de autorização reconhece e confina. A superfície de tools cresceu **+3 259
bytes** (468 807 → 472 066).

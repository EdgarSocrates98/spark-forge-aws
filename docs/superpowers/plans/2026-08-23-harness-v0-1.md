...
> **Correção registrada na execução de I3 (2026-08-23).** A frase acima — "`authorize_tool` tem chamador" — é **falsa**, e o texto original fica como estava porque este plano é registro histórico. Busca exaustiva na implementação: os únicos chamadores são `tests/test_agent_autonomy.py` e o teste de compatibilidade; **zero em produção**. A decisão de manter a assinatura continua certa, mas pelo argumento correto: `authorize_tool` é superfície pública exportada em `sparkforge.agents.__all__`, e adição não é migração. A mesma afirmação foi corrigida em `docs/harness/AUTHORIZATION-CHAIN.md` e na docstring de `TestCompatibilidade`.
>
> **Segunda correção, do mesmo dia.** O Step 1 deste plano mediu `readOnly=True openWorld=True -> 7 tools` e concluiu que `CLOUD_MUTATION` e `DESTRUCTIVE` nasceriam vazias. A medição do catálogo estava certa; a **anotação** é que mentia. Os sete `collect_*` declaravam `readOnlyHint: True` e escrevem em disco (artefato + manifesto de integridade, via `sparkforge.collect.aws._write_and_register`). I3 corrigiu a anotação, e a distribuição real passou a ser `READ_ONLY 32 / LOCAL_MUTATION 5 / CLOUD_MUTATION 7`, com **`CLOUD_READ` e `DESTRUCTIVE`** vazias — duas classes vazias ainda, mas não as que o plano previu. Derivar a classe da anotação foi o que expôs a mentira, que é o valor da fase; ver `docs/harness/AUTHORIZATION-CHAIN.md`.

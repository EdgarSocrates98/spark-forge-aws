...
**D-5d-43** (revisão final, ressalva 2 — a P0 que disparava sobre pré-init sem worker) — `measures.initial_capacity_worker_type_count` contava **entradas lidas do map `initialCapacity`**, e as duas regras de custo liam esse número. Reproduzido antes de decidir: `initialCapacity: {DRIVER: {workerCount: 0, ...}}` com `autoStopConfiguration.enabled: false` disparava **`SF-EMRS-001` em P0**, e o mesmo payload sem `workerCount` disparava **P0 no mesmo julgamento em que saía `emrs.unresolved{missing_worker_count}`**. `SF-EMRS-005` tinha o mesmo termo e caía igual (medido em P2 com janela de 4320). A `explanation` de `SF-EMRS-001` funda o achado em *"o que se cobra é worker existente"* — zero workers não é worker existente, e entrada sem contagem não é worker **provado**. **Escolhi endurecer, não declarar o limite**, e o argumento é do próprio módulo: `knowledge/emr-serverless/application-configuration.md:59` registra `workerCount` apenas como *"(número)"*, sem `Required` e sem mínimo, então não há fonte coletada que torne o estado inalcançável; e a doutrina que sustentou `unknown_capacity_unit` — *"nada na documentação garante que `get-application` só devolva valores que os satisfaçam"* — vale aqui inteira. O que fiz: o measure virou `measures.initial_capacity_worker_count`, **soma dos `workerCount` LIDOS**, e as duas regras leem `>= 1`. Somar em vez de filtrar por `workerCount >= 1` foi decisão: os dois se comportam igual na fronteira `>= 1`, mas a soma é quantidade real que uma regra futura pode limiar, enquanto a contagem filtrada só serve para ser comparada com um. **Não acrescentei uma segunda measure com a regra exigindo as duas**, que era a outra saída oferecida: `soma >= 1` implica `entradas >= 1`, então o segundo termo seria redundante e **apagável sem nenhum golden ficar vermelho** — exatamente o defeito que a verificação de apagabilidade desta fase existe para pegar. A contagem de entradas não se perdeu: ela já era `emrs.analyzed.measures.initial_capacity_count`, e mantê-la também em `emrs.application` seria measure duplicado sem consumidor, do tipo que a `SF-EMR-009` fechou em 81c4744. Duas fixtures novas travam o comportamento, cada uma com dois payloads (um por regra, porque as duas condições de auto-stop são mutuamente exclusivas): `preinit_com_zero_workers` e `preinit_sem_worker_count`. A segunda trava também a **deduplicação do ponto cego**: com `maximumCapacity` completa e `blind=True`, `_capacity_decision` omite o veredito e **não** emite um segundo `unresolved` pelo mesmo buraco. Goldens regenerados por `scripts/regen_fixtures.py`; 14 `facts.json` mudaram, e 3 `findings.json` (o measure vai na evidência do achado).

**D-5d-44** (revisão final, ressalva 5 — o buraco de catálogo, confirmado e reduzido em um) — a revisão mediu "6 das 7 regras com `severity_by` têm ao menos um ramo sem golden". Remedido sobre os 132 `findings.json`: **7 regras, 15 ramos, 9 vistos, 6 faltando** — e a contagem por regra bate (`SF-EMR-006` P2, `SF-ICE-001` P2, `SF-PQ-001` P0 **e** P1, `SF-UI-001` P2, `SF-UI-004` P2; só `SF-EMR-009` fixava os dois). Fechei o da própria área, e ele saiu mais barato do que a ressalva previu: em vez de uma fixture com janela entre 1440 e 10079, **duas payloads no mesmo diretório**, `1440` e `1439`, que fecham o ramo P2 **e** fixam o limiar. Isso importa porque o limiar de `SF-EMRS-005` é `field-heuristic` — a AWS não declara ponto nenhum entre 1 e 10080 —, e com goldens só em 10080 e 15 (`app_saudavel`) um limiar de 720 ou de 4320 passaria sem nenhum vermelho. É a mesma forma do par `auto_termination_idle_day` × `auto_termination_near_threshold` da 5b. Os outros **cinco** ramos ficam registrados como dívida em `STATUS.md`, com a razão de não fechá-los aqui: são de áreas que esta rodada não tocou, e escrever payload de Parquet, Iceberg ou Spark UI sem o corpus daquelas áreas na mão seria inventar em vez de medir. A dívida nomeia também o teste que falta e que tornaria a lacuna visível em vez de silenciosa — varredura que exija golden por ramo, no espírito de `test_fixtures_kind_coverage.py`.

**D-5d-45** (revisão final, ressalva 1 — a contagem de URLs difere da do prompt por uma) — a ressalva diz "13 URLs de Serverless estão no lock (37 → 51)". Medido no diff de `7303ce9~1` para `HEAD`: **14 URLs entraram**, e 51 − 37 = 14. Treze são de domínio Serverless (`docs.aws.amazon.com/emr-serverless/...` e `.../EMR-Serverless-UserGuide/...`); a décima quarta é `https://aws.amazon.com/emr/pricing/`, que `SF-EMRS-001` e `SF-EMRS-005` citam pelo modelo de cobrança de worker e que não existia no lock antes. As duas leituras são verdadeiras — "13 de Serverless" e "14 no total" —, e escrevi a segunda em `knowledge/INDEX.md` porque é a que fecha com a contagem de `sources.lock.json`. Medi junto o que a frase antiga não dizia: das **24** URLs citadas nas seções `## Fontes` dos dois arquivos de `emr-serverless/`, **10 continuam fora do lock** porque regra nenhuma as cita — e isso é o desenho do lock (`watchlist` deriva do catálogo), não lacuna. A frase nova diz as duas coisas.

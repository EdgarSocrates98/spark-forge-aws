...
| código | quando | fase |
|---|---|---|
| `schema_invalid` | frontmatter ausente, não UTF-8, YAML quebrado (com a linha), fora do schema, `phase`/`feature` diferentes do arquivo/pasta; `upstream` em `explore`, ou no `define` sem `explore.md`; `change_kinds` fora do mapa; `verified_by.kind: test` sem `::nome`; `proof` ou `moved` no dev; `evidence` no ship dev; `proof` `finding` sem `#<rule_id>`; `guard` vazio, só com espaço ou fora de texto | todas |

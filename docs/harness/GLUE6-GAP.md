...
| Conhecimento de Iceberg 1.11 (§17) | NÃO EXISTE | A matriz declara a versão de biblioteca do Glue 6.0. Nada descreve remote scan planning, LIMIT pushdown, Partition Stats Scan ou os demais itens da §17 | — |
| Domínio `iceberg-v3`, Variant, deletion vectors e row lineage (§18, §21, §22, §23) | NÃO EXISTE | — | — |
...
| Pipeline nomeado da §32 | EXISTE PARCIAL | As etapas existentes são as regras `SF-MIG` mais os gates de compatibilidade, dados, performance e custo. Etapas de Lake Formation, cross-account, IAM/KMS, rede e consumidor não existem no contrato | `tests/test_migration_assessment.py` |
| Entradas opcionais da §31 (Terraform, JAR, topologia de Lake Formation, inventário de consumidores) | EXISTE PARCIAL | Cada extrator existe isolado; `assess()` recebe uma lista de `Fact` já extraída e não sabe compor as fontes | `tests/test_migration_assessment.py` |
...
| ADR de suporte a Glue 6.0 (§83) | NÃO EXISTE | `docs/vnext/adrs/` é o diretório onde entraria | — |

...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **730** nesta árvore.
...
| `project_items` | 1 | 193 | 353641 | 2201 | 52 |
| `tool_class` | 1 | 187 | 378339 | 3529 | 73 |
| `authorize` | 2 | 376 | 475191 | 4426 | 49 |

Somadas as cinco perguntas: o índice devolve **1688** bytes; ler os arquivos custaria **2168342**;
...
**Contra o denominador do plano, o índice economiza 1284.6 vezes.** Contra a saída de um `grep`
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **227223** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**11356** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que
...
| Controle de verbosidade na resposta | EXISTE PARCIAL | `detail_level` aparece em **40** das **103** tools do catálogo: as que devolvem facts. As duas que paginam e ficaram de fora devolvem outro shape — `sparkforge_judge` devolve findings e `sparkforge_rules_lookup` devolve regras, e nenhum dos dois tem `provenance` nem os campos que o `summary` de fato preserva | `tests/test_adapters_detail_level.py` |

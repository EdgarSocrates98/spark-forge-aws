...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **620** nesta árvore.
...
| `authorize` | 4 | 897 | 407244 | 4426 | 107 |

Somadas as cinco perguntas: o índice devolve **2210** bytes; ler os arquivos custaria **1684664**;
...
**Contra o denominador do plano, o índice economiza 762.3 vezes.** Contra a saída de um `grep`
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **202504** bytes. O `grep` pela definição contendo o fragmento continua menor

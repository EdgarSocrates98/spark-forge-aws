...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **424** nesta árvore.
...
| `project_items` | 1 | 193 | 192625 | 1826 | 52 |
| `tool_class` | 1 | 188 | 28525 | 2563 | 74 |
| `authorize` | 4 | 897 | 228313 | 4426 | 107 |

Somadas as cinco perguntas: o índice devolve **2209** bytes; ler os arquivos custaria **1125405**;
...
**Contra o denominador do plano, o índice economiza 509.5 vezes.** Contra a saída de um `grep`
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  `buscar(banco, "source")` devolve **38** símbolos em **9840** bytes; a saída do `grep` pelo nome,
  no mesmo corpus, tem **116206** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**7068** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que
...
| Controle de verbosidade na resposta | EXISTE PARCIAL | `detail_level` aparece em **25** das **55** tools do catálogo: as que devolvem facts. As duas que paginam e ficaram de fora devolvem outro shape — `sparkforge_judge` devolve findings e `sparkforge_rules_lookup` devolve regras, e nenhum dos dois tem `provenance` nem os campos que o `summary` de fato preserva | `tests/test_adapters_detail_level.py` |
...
existe em **26** delas, e os quatro estimadores de token e os três empacotadores de contexto

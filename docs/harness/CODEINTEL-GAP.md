...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **774** nesta árvore.
...
|---|---|---|---|---|---|
| `iter_source_files` | 2 | 466 | 807855 | 12027 | 102 |
...
Somadas as cinco perguntas: o índice devolve **1688** bytes; ler os arquivos custaria **2258933**;
a saída do `grep` pelo nome, **24906**; a saída do `grep` pela definição, **361**.
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **242888** bytes. O `grep` pela definição contendo o fragmento continua menor

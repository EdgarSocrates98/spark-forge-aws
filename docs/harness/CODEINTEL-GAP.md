...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **472** nesta árvore.
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **133214** bytes. O `grep` pela definição contendo o fragmento continua menor

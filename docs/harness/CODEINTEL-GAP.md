...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **470** nesta árvore.
...
|---|---|---|---|---|---|
| `iter_source_files` | 1 | 466 | 611229 | 8648 | 102 |
| `looks_like_secret` | 2 | 466 | 139575 | 2607 | 85 |
| `project_items` | 1 | 193 | 215178 | 1827 | 52 |
| `tool_class` | 1 | 188 | 28525 | 2563 | 74 |
| `authorize` | 4 | 897 | 284892 | 4426 | 107 |

Somadas as cinco perguntas: o índice devolve **2210** bytes; ler os arquivos custaria **1279399**;
a saída do `grep` pelo nome, **20071**; a saída do `grep` pela definição, **420**.
...
**Contra o denominador do plano, o índice economiza 578.9 vezes.** Contra a saída de um `grep`
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **133006** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**8406** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que
...
| Controle de verbosidade na resposta | EXISTE PARCIAL | `detail_level` aparece em **26** das **63** tools do catálogo: as que devolvem facts. As duas que paginam e ficaram de fora devolvem outro shape — `sparkforge_judge` devolve findings e `sparkforge_rules_lookup` devolve regras, e nenhum dos dois tem `provenance` nem os campos que o `summary` de fato preserva | `tests/test_adapters_detail_level.py` |
...
existe em **27** delas, e os quatro estimadores de token e os três empacotadores de contexto

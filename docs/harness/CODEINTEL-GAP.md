...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **562** nesta árvore.
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **161179** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**9146** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que

...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **751** nesta árvore.
...
| `project_items` | 1 | 193 | 362044 | 2202 | 52 |
| `tool_class` | 1 | 187 | 386742 | 3529 | 73 |
| `authorize` | 2 | 376 | 486259 | 4426 | 49 |

Somadas as cinco perguntas: o índice devolve **1688** bytes; ler os arquivos custaria **2196216**;
...
**Contra o denominador do plano, o índice economiza 1301.1 vezes.** Contra a saída de um `grep`
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **230769** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**11615** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que
...
| Controle de verbosidade na resposta | EXISTE PARCIAL | `detail_level` aparece em **40** das **106** tools do catálogo: as que devolvem facts. As duas que paginam e ficaram de fora devolvem outro shape — `sparkforge_judge` devolve findings e `sparkforge_rules_lookup` devolve regras, e nenhum dos dois tem `provenance` nem os campos que o `summary` de fato preserva | `tests/test_adapters_detail_level.py` |

...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **791** nesta árvore.
...
| `project_items` | 1 | 193 | 367869 | 2202 | 52 |
| `tool_class` | 1 | 187 | 392567 | 3529 | 73 |
| `authorize` | 2 | 376 | 494022 | 4426 | 49 |

Somadas as cinco perguntas: o índice devolve **1688** bytes; ler os arquivos custaria **2262415**;
...
**Contra o denominador do plano, o índice economiza 1340.3 vezes.** Contra a saída de um `grep`
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  `buscar(banco, "source")` devolve **50** símbolos em **12306** bytes; a saída do `grep` pelo nome,
  no mesmo corpus, tem **256880** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**11783** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que

...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **672** nesta árvore.
...
|---|---|---|---|---|---|
| `iter_source_files` | 2 | 466 | 761811 | 11493 | 102 |
| `looks_like_secret` | 2 | 466 | 198731 | 2722 | 85 |
| `project_items` | 1 | 193 | 321960 | 2201 | 52 |
| `tool_class` | 1 | 188 | 28525 | 3181 | 74 |
| `authorize` | 4 | 897 | 442439 | 4426 | 107 |

Somadas as cinco perguntas: o índice devolve **2210** bytes; ler os arquivos custaria **1753466**;
a saída do `grep` pelo nome, **24023**; a saída do `grep` pela definição, **420**.
...
**Contra o denominador do plano, o índice economiza 793.4 vezes.** Contra a saída de um `grep`
pelo nome, **10.9** vezes. E contra a saída de um `grep` pela definição o resultado se inverte: a
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  no mesmo corpus, tem **222889** bytes. O `grep` pela definição contendo o fragmento continua menor
...
| Controle de verbosidade na resposta | EXISTE PARCIAL | `detail_level` aparece em **39** das **94** tools do catálogo: as que devolvem facts. As duas que paginam e ficaram de fora devolvem outro shape — `sparkforge_judge` devolve findings e `sparkforge_rules_lookup` devolve regras, e nenhum dos dois tem `provenance` nem os campos que o `summary` de fato preserva | `tests/test_adapters_detail_level.py` |

...
lados — os arquivos `*.py` que `iter_source_files(root, "*.py")` entrega, **584** nesta árvore.
...
- **O denominador C só funciona se você já souber o nome inteiro e certo.** Para fragmento, o
  ...
  `buscar(banco, "source")` devolve **50** símbolos em **12992** bytes; a saída do `grep` pelo nome,
  no mesmo corpus, tem **196504** bytes. O `grep` pela definição contendo o fragmento continua menor
  (**10743** bytes), mas responde outra coisa — ele lista linhas de definição, e não diz que

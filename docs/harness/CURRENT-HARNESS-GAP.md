...
| **ToolPolicy** | EXISTE PARCIAL | `sparkforge/registry/models.py:ToolManifest.mutation_class: RiskLevel` classifica tool em 4 níveis (`read_only/reversible/sensitive/destructive`) — mais grosseiro que os 5 níveis do §40 (`READ_ONLY/LOCAL_MUTATION/CLOUD_READ/CLOUD_MUTATION/DESTRUCTIVE`), mas cobre a mesma ideia. `sparkforge/agents/autonomy.py:AutonomyController.authorize_tool()` já aplica a regra "mutação exige approval" (`tests/test_agent_autonomy.py`). O que falta: nenhum dos dois é **enforced** — não há hook `PreToolUse` (ver seção Hooks abaixo) barrando uma tool `DESTRUCTIVE` de fato rodar; a política existe como dado e como função pura, não como gate executado. | `tests/test_agent_autonomy.py::test_stop_on_stagnation_and_authorization_policy` (só a função `authorize_tool`, não enforcement real) |
| **ExecutionState** | EXISTE, com teste | `sparkforge/case/store.py`: `PHASES = (intake, inventory, facts, diagnosis, hypothesis, experiment, validation, report)` + `GATES = (baseline_captured, dominant_bottleneck_identified, functional_validation_defined, flows_mapped)`, com `strict_gates` fail-closed via `rules/catalog/routing.yaml:gates`. Cobre o §36 quase 1:1 — falta só o mapeamento nominal para os estados do harness (`CREATED/PLANNED/RUNNING/WAITING_APPROVAL/...`), que são um relabeling do que já existe. | `tests/test_case_store.py` (667 linhas), `tests/test_case_router.py` |
| **EvaluationGate** | EXISTE PARCIAL | Dois sistemas distintos e não integrados: (1) `sparkforge/evals/runner.py:EvaluationRunner.run_router_eval()` mede acurácia do router contra `sparkforge/evals/datasets/router_dataset.json` — só **4 casos**, não os 5 casos de ouro que o §48 pede; (2) `evals/fase0.xml` + `scripts/check_evals.py` é um segundo harness de avaliação, baseado em pares pergunta/resposta verificáveis contra fixtures — 10 pares. Nenhum dos dois é um "gate" que bloqueia merge/release; são scripts chamados manualmente/por teste. | `tests/test_eval_runner.py`, `tests/test_evals.py` |
...
| **Authorization chain** (§76) | NÃO EXISTE | `authorize_tool()` é uma checagem de um nível (agente × tool × approval booleano). Não há cadeia (quem autorizou, sob qual perfil, com qual escopo) nem propagação por `ExecutionProfile`/`RiskLevel` de ponta a ponta. | — |
...
| **Prompt injection** (§42) | NÃO EXISTE | Nenhum módulo trata conteúdo de repositório/log/AWS/MCP/web como dado não-executável de forma explícita e testada. `sparkforge/adapters/mcp.py` expõe tools mas não há sanitização/gate documentado para esse vetor. | — |
...
| **Runtime harness vs evaluation harness** (§43, "crítico") | EXISTE PARCIAL, e já separado na prática | O runtime (CLI `sparkforge analyze/judge/case/next/resume`, `sparkforge/case/router.py`) e a avaliação (`sparkforge/evals/`, `evals/fase0.xml` + `scripts/check_evals.py`, `tests/test_fixtures_golden_*.py` — 24 arquivos de golden fixtures) já são pastas e mecanismos **fisicamente separados**, o que é a exigência central do §43. O que falta é a formalização: nenhum documento nomeia essa fronteira como "Runtime Harness" vs "Evaluation Harness" nem declara o contrato entre os dois (quem alimenta quem). | `tests/test_fixtures_golden_migration.py` e as outras 23 famílias `test_fixtures_golden_*` são o corpo do "evaluation harness" de fato; `tests/test_case_router.py`/`tests/test_case_store.py` são o "runtime harness" de fato |
| **Grader hierarchy** (§45) | EXISTE, com teste — nunca formalizado como hierarquia | Toda a suíte de `tests/test_fixtures_golden_*.py` (24 arquivos) é grader **code-based determinístico** contra fixtures — o nível 1 do §45, e o único usado hoje. Não há grader LLM nem grader de state verification separado — o que, dado o §45 ("nunca usar LLM grader para algo que pytest pode verificar melhor"), é possivelmente já o comportamento correto, não uma lacuna. | Os 24 arquivos `tests/test_fixtures_golden_*.py` |
| **Golden cases do §48** | EXISTE PARCIAL | Caso 1/2 (Glue 4.0→5.1, Parquet e Iceberg) e caso 4 (Terraform, mudança destrutiva) têm fixtures e testes próprios (`test_fixtures_golden_migration.py`, `test_fixtures_golden_terraform.py`, `test_fixtures_golden_tfdiff.py`). Caso 3 (Lake Formation cross-account) tem `test_lakeformation_engine.py`, mas é um teste de 8 símbolos/1924 bytes — muito mais raso que os outros golden suites, sem fixture dedicada equivalente. Caso 5 (Spark performance, event log + plano) tem fixtures separadas para plano (`test_fixtures_golden_plan.py`) e event log (`test_fixtures_golden_eventlog.py`) mas não um golden case único que combine os dois como o §48 pede. Nenhum dos 5 está reunido sob um dataset de avaliação nomeado "golden cases do harness" — cada um vive como suíte de teste de unidade/fixture, não como caso de harness. | ver arquivos citados |
...
- **`sparkforge/adapters` é o maior pacote e tem um gate de paridade que roda.**
  Confirmado por tamanho: `_core.py` (120,7 KB) e `tools.py` (118,5 KB) são, disparado, os dois maiores arquivos do pacote determinístico — maiores que todo `sparkforge/economy`, `sparkforge/context`, `sparkforge/registry`, `sparkforge/case`, `sparkforge/agents` e `sparkforge/workflows` somados. O gate de paridade é real e pesado: `tests/test_capability_parity.py` (713 linhas), `tests/test_agents_parity.py` (539 linhas) e `tests/test_platform_divergence.py` (227 linhas) — 1479 linhas de teste só para paridade entre adapters/plataformas. O histórico de commits confirma que esse gate já pegou regressão real (`2cc7d27 fix(ci): o gate de paridade do artefato nao alcancava scripts/, e estava vermelho desde 9474aa8`).
...
- **Unificar os três budgets** (`TaskBudgetGuardrail`, `AutonomyBudget`, `Supervisor.Budget`)
  ...
  de três.
- **Enforcement real de tool policy via hook `PreToolUse`** (§41): a política existe como
  dado (`ToolManifest.mutation_class`) e como função pura (`authorize_tool`), mas nada
  a torna obrigatória no momento em que uma tool destrutiva de fato executaria. Isso é
  gap genuíno, não duplicação.
...
- **`HarnessContract` como classe** (§15) — hoje só tem um consumidor real
  ...
  estaria generalizando a partir de N=1.

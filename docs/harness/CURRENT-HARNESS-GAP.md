...
- **`sparkforge/adapters` é o maior pacote e tem um gate de paridade que roda.**
  Confirmado por tamanho: `tools.py` (148,0 KB) e `_core.py` (137,2 KB) são, disparado, os dois maiores arquivos do pacote determinístico — maiores que todo `sparkforge/economy`, `sparkforge/context`, `sparkforge/registry`, `sparkforge/case`, `sparkforge/agents` e `sparkforge/workflows` somados. O gate de paridade é real e pesado: `tests/test_capability_parity.py` (749 linhas), `tests/test_agents_parity.py` (539 linhas) e `tests/test_platform_divergence.py` (227 linhas) — 1515 linhas de teste só para paridade entre adapters/plataformas. O histórico de commits confirma que esse gate já pegou regressão real (`2cc7d27 fix(ci): o gate de paridade do artefato nao alcancava scripts/, e estava vermelho desde 9474aa8`).
...
- **Enforcement real de tool policy via hook `PreToolUse`** (§41): a política existe como
  dado (`ToolManifest.mutation_class`) e como função pura (`authorize()`, que desde a
  Fase J2 também examina o caminho recebido nos argumentos), mas nada a torna
  obrigatória no momento em que uma tool destrutiva de fato executaria — nenhum dos
  quatro caminhos de execução (`adapters/mcp.py`, `adapters/tools.py`, `adapters/cli.py`,
  `agents/supervisor.py`) chama `authorize()`. Isso é gap genuíno, não duplicação, e a
  decisão de projeto que o bloqueava (a cadeia recebe `arguments`, ou só o hook os vê?)
  está tomada.

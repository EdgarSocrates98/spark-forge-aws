...
| **ToolPolicy** | EXISTE, com teste | Fase I3: `sparkforge/agents/autonomy.py:tool_class()` deriva os 5 níveis do §40 (`READ_ONLY/LOCAL_MUTATION/CLOUD_READ/CLOUD_MUTATION/DESTRUCTIVE`) das anotações MCP que cada tool já declara em `sparkforge/adapters/tools.py` (`readOnlyHint`/`openWorldHint`/`destructiveHint`) — e não de uma segunda lista mantida à mão. Duas classes nascem vazias — nenhuma tool é `destructive`, nenhuma é mutante e de nuvem —, e isso é o resultado, não lacuna: o valor da classificação é impedir que uma tool futura entre sem classe. `tool_class()` levanta `KeyError` para nome desconhecido, em vez de devolver `READ_ONLY`. Documentado em [`AUTHORIZATION-CHAIN.md`](AUTHORIZATION-CHAIN.md). Continua existindo, em paralelo e sem unificação, `sparkforge/registry/models.py:ToolManifest.mutation_class: RiskLevel`, que classifica tool em 4 níveis (`read_only/reversible/sensitive/destructive`) — mais grosseiro, e não consultado pela derivação. O que continua fora: o enforcement do §41 — não há hook `PreToolUse` barrando uma tool `DESTRUCTIVE` de fato rodar; a política é decidida, não imposta. | `tests/test_harness_authorization.py`, `tests/test_agent_autonomy.py` |

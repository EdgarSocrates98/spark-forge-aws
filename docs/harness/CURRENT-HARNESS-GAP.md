...
Achado central da investigação: **o repositório tinha duas implementações paralelas
de várias das mesmas ideias** — `sparkforge/economy/router.py:CapabilityModelRouter`
e `sparkforge/agents/model_policy.py:ModelSelector` eram dois model routers distintos;
`sparkforge/workflows/spec.py:TaskSpec` e `sparkforge/agents/supervisor.py` cobriam
pedaços sobrepostos de execução em fases; `sparkforge/economy/budget.py:TaskBudgetGuardrail`
e `sparkforge/agents/autonomy.py:AutonomyBudget` eram orçamentos de execução com formas
diferentes. Em 2026-09-15 as duplicatas sem uso em produção saíram (`ModelSelector`,
`Supervisor`, `TaskBudgetGuardrail`, `AutonomyBudget`): ficam `CapabilityModelRouter` e a
família de `sparkforge/agentic/budget.py`, como o §100 do harness pedia.
...
| **ExecutionPlanner** | EXISTE PARCIAL | `sparkforge/workflows/dag.py:ExecutionDAG.compute_waves()` faz exatamente a §14 (dependency-aware waves, detecção de ciclo). O plano de sete fases fixas do `Supervisor` (`sparkforge/agents/supervisor.py`) foi removido em 2026-09-15, sem uso em produção; sobra o DAG genérico, e nada monta `ExecutionPlan` como pede o §13/§14. | `tests/test_workflows_dag.py::test_execution_dag_waves`, `::test_execution_dag_cycle_detection` |
...
|---|---|---|---|
| **ModelRouter** | EXISTE, com teste | `sparkforge/economy/router.py:CapabilityModelRouter.route_task()` (tier determinístico-primeiro, keyword-matched specialist skills, `ModelTier` de `TIER_0_DETERMINISTIC` a `TIER_6_MULTI_AGENT` — literalmente o §33/§34, incluindo "NONE é modelo válido" via `TIER_0_DETERMINISTIC` com `max_tokens=0`, `estimated_cost_usd=0.0`). Havia um **segundo** router, `sparkforge/agents/model_policy.py:ModelSelector`, orientado a inventário de contas e sem uso em produção; foi removido em 2026-09-15. | `tests/test_economy_engine.py::test_capability_router_deterministic_first`, `::test_capability_router_specialist_skill` |
...
| **RecoveryEngine** | NÃO EXISTE | O único critério de parada que existia, `AutonomyController.should_stop()`, saiu em 2026-09-15 sem uso em produção; o que para hoje é o budget declarado (`CaseBudget`/`DebateBudget`) e as recusas nomeadas de cada verbo. Não há classificação de *tipo* de falha (`missing evidence / tool error / model error / invalid output / environment / permission / unsupported case`, §31) nem lógica de retry condicionada ao tipo (§32). | — |
...
| **AgentFanoutGovernor** | EXISTE PARCIAL | `CaseBudget.max_agents` e `can_spawn_agent()` (`sparkforge/agentic/budget.py`) limitam fanout numericamente, lidos do bloco `budget:` do case. Falta a política declarativa por `ExecutionProfile` que o §25 pede (`simple task: agents=0`, `critical cross-domain: agents<=2`) — hoje o limite é um número fixo por instância de budget, não uma regra condicionada a `RiskLevel`/`ExecutionProfile`. | `tests/test_agentic_infra.py` |
...
| **Advanced planning** | NÃO EXISTE | Nada além do DAG de waves (já listado acima). Sem replanning condicionado a resultado parcial. | — |

## 3. Preocupações transversais que o documento trata à parte

| Conceito | Classificação | Módulo(s) | Teste |
|---|---|---|---|
| **Routing L0–L4** (§11) | EXISTE PARCIAL | `sparkforge/case/router.py:next_step()` sobre `rules/catalog/routing.yaml` (40 KB) é um roteador determinístico completo: condições declarativas (`equals/absent/present/count_gt/count_eq/contains/any_where`), regras `ROUTE-*` (skill) e `AGENT-*` (coordenador) avaliadas na mesma engine, `blocked_by` por gate. Isso é o **L0 determinístico** do §11, robusto e testado. Os níveis L1–L4 (Skill route → specialist → reviewer → multi-agent) não têm um router formal equivalente — a escalada hoje é decisão do agente/coordenador em Markdown (`agents/*.md`), não código. | `tests/test_case_router.py`, mais os testes de `rules/catalog/routing.yaml` via `tests/test_rules_*` |
...
| **Early exit** (§30) | EXISTE, com teste | `ModelTier.TIER_0_DETERMINISTIC` no router (custo 0, sem chamada de modelo) implementa a condição do §30 na prática, embora não exista uma função única chamada `early_exit()`. | `tests/test_economy_engine.py::test_capability_router_deterministic_first` |
| **Failure attribution** (§79) / **Failure spend governance** (§31) | NÃO EXISTE | Nada classifica *por quê* uma execução parou nas sete categorias do §31 (`missing evidence/tool error/model error/invalid output/environment/permission/unsupported case`). Nenhum módulo produz esse enum. | — |
| **Tool allowlist** (§77) | EXISTE, com teste — **superfície diferente do harness** | `tests/test_execution_surface.py` já implementa uma allowlist fechada e testada, mas para **hooks e servidores MCP** (`.claude/settings.json`, `vendor/caveman/.claude-plugin/plugin.json`, `.mcp.json`) — não para tools MCP individuais do próprio Spark Forge. A allowlist por agente é `CallPolicy.allowed_tools` (`sparkforge/agents/autonomy.py`), imposta em `call_tool` quando há política declarada. | `tests/test_execution_surface.py` (allowlist de hooks/MCP), `tests/test_harness_authorization.py` (allowlist por agente) |
...
| **Claude Code hooks para enforcement** (§41) | EXISTE desde 2026-09-13 (§16), com teste | `.claude/settings.json` ganhou o `PreToolUse` `python -m sparkforge.policy.hook` (matcher `Bash\|Edit\|Write\|MultiEdit\|NotebookEdit`), que lê `.sparkforge/policy.yaml` e bloqueia as regras `deny` pelo código de saída de bloqueio; as regras `ask` (`terraform apply`/`destroy`, `aws s3 rm`, `lakeformation revoke-permissions`, `expire_snapshots`, escrita em `rules/catalog/**` e `*.tf`, tools `CLOUD_MUTATION`) viram `permissions.ask`, porque o `PreToolUse` só decide `allow`/`deny`. Limite declarado: regra casa o texto do comando, não o programa. | `tests/test_fixtures_golden_policy.py` (o hook por subprocess), `tests/test_policy_decide.py`, `tests/test_execution_surface.py` (o comando na lista fechada) |
...
| **Grader hierarchy** (§45) | EXISTE, com teste — nunca formalizado como hierarquia | Toda a suíte de `tests/test_fixtures_golden_*.py` (57 arquivos) é grader **code-based determinístico** contra fixtures — o nível 1 do §45, e o único usado hoje. Não há grader LLM nem grader de state verification separado — o que, dado o §45 ("nunca usar LLM grader para algo que pytest pode verificar melhor"), é possivelmente já o comportamento correto, não uma lacuna. | Os 57 arquivos `tests/test_fixtures_golden_*.py` |
...
- **`sparkforge/adapters` é o maior pacote e tem um gate de paridade que roda.**
  Confirmado por tamanho: `tools.py` (455,5 KB) e `_core.py` (356,7 KB) são, disparado, os dois maiores arquivos do pacote determinístico — maiores que todo `sparkforge/economy`, `sparkforge/context`, `sparkforge/registry`, `sparkforge/case`, `sparkforge/agents` e `sparkforge/workflows` somados. O gate de paridade é real e pesado: `tests/test_capability_parity.py` (930 linhas), `tests/test_agents_parity.py` (573 linhas) e `tests/test_platform_divergence.py` (227 linhas) — 1730 linhas de teste só para paridade entre adapters/plataformas. O histórico de commits confirma que esse gate já pegou regressão real (`2cc7d27 fix(ci): o gate de paridade do artefato nao alcancava scripts/, e estava vermelho desde 9474aa8`).
...
- **Unificar os budgets e os model routers** — feito em 2026-09-15, por remoção: saíram
  `TaskBudgetGuardrail`, `AutonomyBudget`, `Supervisor.Budget` e `ModelSelector`, nenhum
  usado em produção. Fica `CapabilityModelRouter` e a família de `sparkforge/agentic/budget.py`
  (`AgentBudget`, `CaseBudget`, `DebateBudget`), que já existia como a *quarta* forma de
  orçamento que este item temia — e agora é a única.
- **Enforcement de tool policy FORA do processo Python, via hook `PreToolUse`** (§41):
  ...
  o que cobre as 108 tools num ponto só. O que nunca passava por `call_tool` —
  `terraform destroy` por `Bash` — passou a ser interceptado pelo hook `PreToolUse` da
  policy em 2026-09-13 (§16), com a regra casando texto e não programa. Duas
...
- **`AgentFanoutGovernor` como política declarativa por `ExecutionProfile`** (§25) —
  ...
  de multi-agent além do `Supervisor` de sete fases (removido em 2026-09-15) para provar que a política precisa

...
| T-024 | Exfiltração entre tools | **Parcial** | perfil de execução como vocabulário canônico, classe de tool derivada da anotação e aprovação por classe, em `sparkforge/agents/autonomy.py` | `tests/test_harness_authorization.py::test_OFFLINE_recusa_tool_de_rede_em_qualquer_grafia`; `::test_a_aprovacao_e_por_classe_e_nao_um_booleano_global` | A cadeia passou a ser **imposta** no despacho (`adapters/tools.py:call_tool` via `CallPolicy`), e o handler não roda quando ela recusa. Em 2026-09-13 (§16) as duas lacunas antigas fecharam em parte: a política passou a ser DECLARADA em `.sparkforge/policy.yaml` (commitada neste repositório) e imposta em três portas — o servidor MCP a carrega ao subir (classes pré-aprovadas, `tools.denied`, caminhos confinados ao repositório e a `extra_roots`), o hook `PreToolUse` bloqueia as regras `deny` de `Bash`/`Edit`/`Write`, e as regras `ask` viram `permissions.ask` do `.claude/settings.json` (`tests/test_fixtures_golden_policy.py`, `tests/test_policy_mcp.py`). Continua **Parcial** por um motivo declarado e medido na documentação do Claude Code: regra de shell casa o TEXTO do comando, não o programa — alias, script que chama o programa por dentro e caminho absoluto do binário escapam —, e `bypassPermissions` pula as regras `ask` (o `deny` do hook segue valendo) |

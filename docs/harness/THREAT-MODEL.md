...
| T-024 | Exfiltração entre tools | **Parcial** | perfil de execução como vocabulário canônico, classe de tool derivada da anotação e aprovação por classe, em `sparkforge/agents/autonomy.py` | `tests/test_harness_authorization.py::test_OFFLINE_recusa_tool_de_rede_em_qualquer_grafia`; `::test_a_aprovacao_e_por_classe_e_nao_um_booleano_global` | A cadeia passou a ser **imposta** no despacho (`adapters/tools.py:call_tool` via `CallPolicy`), e o handler não roda quando ela recusa. Continua **Parcial** por dois motivos declarados: a imposição vale dentro do processo Python — `Bash` e `terraform destroy` seguem fora, que é o hook `PreToolUse` do §41 —, e ela só morde onde há política declarada; sem `policy`, o comportamento é o de antes por decisão de não-regressão |

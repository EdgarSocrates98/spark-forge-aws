...
2026-09-19, mas por uma feature diferente (`docs/sdd/CRITERIO_DE_DOMINIO/`): sem regra
executável que julgasse artefato nenhum, o critério de domínio
(`tests/test_criterio_de_dominio.py`) os desqualificou. Só dois continuam existindo como
agentes ativos em `agents/` — `sf-runtime-specialist` e `sf-security-reviewer` — roteados
de fato em `rules/catalog/routing.yaml` e exercitados por `tests/test_router_agents.py`.
...
7. `tool-specialist-routing`: Seleção correta de ferramentas determinísticas por domínio.

    ...
    def choose(self, models: Iterable[ModelInfo], demand: ModelDemand) -> ModelChoice:
        ...
        if demand.max_input_price is not None:
            candidates = [
                m
                for m in candidates
                if m.input_price is None or m.input_price <= demand.max_input_price
            ]
        if demand.max_output_price is not None:
            candidates = [
                m
                for m in candidates
                if m.output_price is None or m.output_price <= demand.max_output_price
            ]
        if not candidates:
        ...
        def score(model: ModelInfo) -> tuple[int, int, float]:
            capability = (
                model.quality
                + (2 if model.reasoning and hard else 0)
                + (1 if model.tools and demand.needs_tools else 0)
            )

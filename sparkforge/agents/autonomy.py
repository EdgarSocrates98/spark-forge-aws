"""Classe de tool e cadeia de autorizacao dos agentes do SparkForge.

O controlador de autonomia que morava aqui (`AutonomyBudget`,
`AutonomyController`, `StopDecision`, `RouteDecision`) saiu em 2026-09-15: nenhum
caminho de producao o usava, e o budget do pacote e a familia de
`sparkforge.agentic.budget`.
"""
...
def tool_class(tool: str) -> ToolClass:
    ...
    quem quisesse `ConversationRoom`; e a leitura em tempo de chamada e o que
...
def authorize(
    *,
    ...
    IMPOE nada. Nenhum dos tres caminhos de execucao (`adapters/mcp.py`,
    `adapters/tools.py`, `adapters/cli.py`) chama

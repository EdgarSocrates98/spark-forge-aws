...
| Como vejo as tools e a sessão no meu backend de tracing? | `telemetry export` | os spans que `call_tool` gravou no ledger e, com `--host-transcript`, o transcript do host; grava OTLP/JSON (`gen_ai.*`, `mcp.*`) em `.sparkforge/telemetry/` para o receiver `otlpjsonfile` do Collector, com o provider declarado e recusa nomeada para span sem horário |

# SparkForge: findings deste commit

| Severidade | Findings | No Code Scanning | Sem localizacao no repositorio |
|---|---|---|---|
| P0 | 1 | 0 | 1 |
| P1 | 1 | 0 | 1 |

**Gate:** `--fail-on P1`: disparou.

## No Code Scanning (0)

Nenhum finding com linha em arquivo do repositorio.

## Sem localizacao no repositorio (2)

| Severidade | Regra | Motivo | Sujeito | Titulo |
|---|---|---|---|---|
| P0 | SF-BENCH-001 | `runtime` | job_run `different_input_volume` | Comparação entre execuções com volumes de entrada diferentes (medido: total_gc_ms_after=1500, total_gc_ms_before=1100, total_gc_ms_delta_pct=36.4, total_input_bytes_after=280000000, total_input_bytes_before=140000000, total_input_bytes_delta_pct=100.0, total_spill_bytes_after=0, total_spill_bytes_before=0, total_task_count_after=10, total_task_count_before=10, total_task_count_delta_pct=0.0, total_task_ms_after=37600.0, total_task_ms_before=22000.0, total_task_ms_delta_pct=70.9) |
| P1 | SF-BENCH-002 | `runtime` | job_run `different_input_volume` | Tempo de task somado piorou entre as duas execuções (medido: total_gc_ms_after=1500, total_gc_ms_before=1100, total_gc_ms_delta_pct=36.4, total_input_bytes_after=280000000, total_input_bytes_before=140000000, total_input_bytes_delta_pct=100.0, total_spill_bytes_after=0, total_spill_bytes_before=0, total_task_count_after=10, total_task_count_before=10, total_task_count_delta_pct=0.0, total_task_ms_after=37600.0, total_task_ms_before=22000.0, total_task_ms_delta_pct=70.9) |
